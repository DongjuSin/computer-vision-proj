...
class JPU(nn.Module):
    def __init__(self, in_channels, width=512, norm_layer=None, up_kwargs=None):
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(in_channels[-1]*2, width, 3, padding=1, bias=False),
            norm_layer(width),
            nn.ReLU(inplace=True))
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels[-2]*2, width, 3, padding=1, bias=False),
            norm_layer(width),
            nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels[-3]*2, width, 3, padding=1, bias=False),
            norm_layer(width),
            nn.ReLU(inplace=True))
        ''' ## original
        self.conv5 = nn.Sequential(
